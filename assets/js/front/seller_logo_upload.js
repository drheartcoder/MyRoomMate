(function () {
	var logoUpload = document.getElementById("user_image"),
		...
		*/
		if (typeof FileReader !== "undefined" && (/image/i).test(file.type)) {
			...
			reader.onload = (function (theImg) {
				return function (evt) {
					...
					$('#user_image_view').attr('src', evt.target.result);
				};
			}(img));
		...
		xhr.upload.addEventListener("progress", function (evt) {
			if (evt.lengthComputable) {
				...
				jQuery("#user_image_loader").erezervo("show");
			}
			else {
		...
		xhr.addEventListener("load", function () {
			jQuery("#user_image_loader").erezervo("hide").delay(1000).hide(0,'',function(){
				$("#user_image_loader").hide();
				$("#user_image_view").show();
