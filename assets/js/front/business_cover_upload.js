(function () {
	var logoUpload = document.getElementById("cover_photo"),
		...
		*/
		if (typeof FileReader !== "undefined" && (/image/i).test(file.type)) {
			...
			reader.onload = (function (theImg) {
				return function (evt) {
					...
					$('#cover_photo_view').attr('src', evt.target.result);
				};
			}(img));
		...
		xhr.upload.addEventListener("progress", function (evt) {
			if (evt.lengthComputable) {
				...
				jQuery("#cover_photo_loader").erezervo("show");
			}
			else {
		...
		xhr.addEventListener("load", function () {
			jQuery("#cover_photo_loader").erezervo("hide").delay(1000).hide(0,'',function(){
				$("#cover_photo_loader").hide();
				$("#cover_photo_view").show();
