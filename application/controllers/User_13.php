 ...
 // end addlisting
	...
	function frombuild()
	{
		...
		if(count($catoptions) > 0 ) {
			...
			<div class="title-redios">Additional Information</div>
			<br>
			</div>
			<?php 
	        ...
	        if($this->master_model->insertRecord('tbl_addlisting',$arr_Data))
	        {
				...
				foreach ($catoptions as $key => $value) {
					...
					if($value['fields_type']=="selectmultiple") {
						...
						$selectmultiple = implode(", ",$_POST[$value['attribute_slug']]);
				...
				$addlisting = $this->master_model->getRecords('tbl_addlisting',array('id'=>$last_inserted_id));
				...
				{ 
					...
					$other_info =array(
					  				...
					  				"listing_id"	       => base_url('listing/details').'/'.$addlisting[0]['slug'],
					  				//"subject"            => $subject,
					  				//"mobile_no"          => $mobile_no,
					  				/*"message"            => $message,*/
		  			...
		  			$info_arr   =array(
							        ...
							        'subject'	         => 'Listing Added Notification Mail - '.PROJECT_NAME,
    ...
    */
	...
	{ 
		...
		if($resArray['ACK']=="Success")
		{
			$wherelisting = array('id' => $this->session->userdata('last_listing_id'));
    		$this->master_model->updateRecord('tbl_addlisting', array('status' => '1','is_delete' => '0'), $wherelisting);
			...
			$addlisting = $this->master_model->getRecords('tbl_addlisting',array('id'=>$this->session->userdata('last_listing_id')));
    ...
    function edit_mylisting()
    {
		...
		else
		{
	    	if($this->uri->segment(3) != '')
	    	{
				...
				$this->db->group_by('tbl_addlisting_data.attribute_id'); 
		   	    $this->db->order_by('tbl_category_form_fields.orderby','ASC');
