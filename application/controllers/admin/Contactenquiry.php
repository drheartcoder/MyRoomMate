...
class Contactenquiry extends CI_Controller
{
	...
	public function manage()
	{
		...
		$config['total_rows']=$this->master_model->getRecordCount('tbl_contact_us');
		$config['num_links']	=	3;
		...
		$enquiry_list=array();
		$enquiry_list=$this->master_model->getRecords('tbl_contact_us','','*',array('contact_id'=>'DESC'),$page,$config['per_page']);
	...
	{ //echo"hh";exit;
		...
		if($contact_id!='')
		{
			$del=$this->master_model->deleteRecord('tbl_contact_us','contact_id',$contact_id);
			if($del)
			{
				$this->session->set_flashdata('success','Contact Enquiry Deleted Successfully');
				redirect(base_url().ADMIN_PANEL_NAME.'contactenquiry/manage/');
			}
			else
			{
		...
		if(isset($_POST['multiple_action']) && isset($_POST['chk_enquiry']))
		{
			if(count($_POST['chk_enquiry'])>0)
			{
				if($_POST['multiple_action']=='delete')
				{
					...
					{
						$this->master_model->deleteRecord('tbl_contact_us','contact_id',$row);
					}
				}
				$this->session->set_flashdata('success','Contact Enquiry Deleted Successfully.');
				redirect(base_url().ADMIN_PANEL_NAME."contactenquiry/manage");
