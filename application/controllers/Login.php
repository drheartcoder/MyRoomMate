...
class Login extends CI_Controller {
	...
	public function resetpassword() {
		...
		if(isset($_POST['newpwd']) && isset($_POST['username']))
		{
			...
			$arr_confcode_check = $this->master_model->getRecords('tbl_user_master',array('username'=>trim($username)));

			if(count($arr_confcode_check) > 0) 
			{
				$this->master_model->updateRecord('tbl_user_master',array("password"=>$encrypted_password,'confirm_code'=>NULL),array('username'=>"'".$username."'"));
			...
			{
				$arr_response['status'] = "error";
				$arr_response['msg']    = "Username not valid..!";
				$arr_response['URL']    = base_url().'home';
				echo json_encode($arr_response);
				exit;
			}
		...
		$data['pageTitle']       ='Reset Password - '.PROJECT_NAME;
   	    $data['page_title']      ='Reset Password - '.PROJECT_NAME;
   	    $data['middle_content']  ='resetpasswordforolduser';
	    $this->load->view('template',$data);
